...
> - ✅ Check \`.ux-ingkarc.json\` for \`auto-resolve\` setting BEFORE working
...
1. ✅ **Check if \`.ux-ingkarc.json\` exists** and read \`auto-resolve\` setting
...
1. **Check if \`.ux-ingkarc.json\` exists** in project root
...
\`\`\`javascript
const fs = require('fs');
const config = fs.existsSync('.ux-ingkarc.json') ? JSON.parse(fs.readFileSync('.ux-ingkarc.json')) : {};
...
### 1. Check for \`.ux-ingkarc.json\` Configuration
...
4. ✅ **CHECK CONFIG:** Read \`.ux-ingkarc.json\` for \`auto-resolve\` setting
...
**Reading this file completely = Following UX Ingka Workflow correctly.**
