...
class ComponentInstaller {
  ...
  async copyComponentTemplates() {
    ...
    for (const componentName of this.selectedComponents) {
      ...
      if (await fs.pathExists(templatePath)) {
        ...
        // Special handling for 'icon' component - also copy the Icons SVG folder
        if (componentName === 'icon' || componentName === 'Icon') {
          await this.installIconsSvgFolder(outputDir);
        }
      } else {
  ...
  /**
   * Install Icons SVG folder (600+ IKEA icons)
   * Called when the 'icon' component is installed
   */
  async installIconsSvgFolder(outputDir) {
    console.log(chalk.cyan('\n🎨 Installing IKEA SVG Icons...\n'));

    const iconsSourceDir = path.join(__dirname, '../../templates/ingka-components/Icons');
    const publicIconsDir = path.join(this.projectRoot, 'public', 'Icons');
    const srcIconsDir = path.join(outputDir, 'Icons');

    // Check if source Icons folder exists
    if (!await fs.pathExists(iconsSourceDir)) {
      console.log(chalk.yellow('   ⚠️  Icons SVG folder not found in templates'));
      return;
    }

    // Copy to public folder (for web serving at /Icons/...)
    try {
      await fs.ensureDir(publicIconsDir);
      await fs.copy(iconsSourceDir, publicIconsDir);

      // Count icons
      const iconFiles = await fs.readdir(publicIconsDir);
      const svgCount = iconFiles.filter(f => f.endsWith('.svg')).length;

      console.log(chalk.green(`   ✓ Installed ${svgCount} SVG icons to public/Icons/`));
    } catch (error) {
      console.log(chalk.yellow(`   ⚠️  Failed to copy to public/Icons: ${error.message}`));
    }

    // Also copy to src/components/ingka/Icons for imports
    try {
      await fs.ensureDir(srcIconsDir);
      await fs.copy(iconsSourceDir, srcIconsDir);
      console.log(chalk.green(`   ✓ Icons also available at src/components/ingka/Icons/`));
    } catch (error) {
      console.log(chalk.yellow(`   ⚠️  Failed to copy to src folder: ${error.message}`));
    }

    console.log(chalk.gray('\n   Usage: <Icon name="heart" /> or <Icon name="arrow-right" />\n'));
  }
