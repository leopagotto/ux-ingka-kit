/**
 * VS Code Extension: UX Ingka Model Selector Display
 *
 ...
 * 1. Copy to: ~/.vscode/extensions/ux-ingka-model-selector/
 * 2. Restart VS Code
...
class LeoModelSelectorExtension {
   ...
   */
  async activate() {
    console.log('UX Ingka Model Selector Extension activating...');
    ...
    // Watch for .ux-ingka-model-status.json changes
    this.watchStatusFile();

    console.log('UX Ingka Model Selector Extension activated!');
  }
  ...
  createStatusBar() {
    ...
    this.statusBar.command = 'ux-ingka-model-selector.showModelInfo';
    this.statusBar.tooltip = 'UX Ingka Model Selector - Click for details';
  ...
  registerCommands() {
    ...
    this.context.subscriptions.push(
      vscode.commands.registerCommand('ux-ingka-model-selector.showModelInfo', async () => {
        const statusFile = path.join(
          require('os').homedir(),
          '.ux-ingka-kit',
          '.ux-ingka-model-status.json'
...
${status.history?.map(h => `  • ${h.event || 'status'}: ${h.agent || 'system'}`).join('\n') || '  (none)'}
    ...
    this.context.subscriptions.push(
      vscode.commands.registerCommand('ux-ingka-model-selector.selectModel', async () => {
        const models = [
    ...
    // Show model history
    this.context.subscriptions.push(
      vscode.commands.registerCommand('ux-ingka-model-selector.showHistory', async () => {
        const statusFile = path.join(
          require('os').homedir(),
          '.ux-ingka-kit',
          '.ux-ingka-model-status.json'
 ...
 */
function deactivate() {
  console.log('UX Ingka Model Selector Extension deactivated');
}
