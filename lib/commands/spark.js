...
/**
 * Install IKEA SVG Icons to public folder
 * Copies 600+ SVG icons for use with Icon component
 */
async function installIkeaIcons(appPath) {
  console.log(chalk.yellow('\n🎨 Installing IKEA SVG Icons...\n'));

  try {
    const iconsSourceDir = path.join(__dirname, '../../templates/ingka-components/Icons');
    const publicIconsDir = path.join(appPath, 'public', 'Icons');

    // Check if source Icons folder exists
    if (!await fs.pathExists(iconsSourceDir)) {
      console.log(chalk.yellow('⚠️  Icons folder not found in templates'));
      return false;
    }
    ...
    console.log(chalk.yellow('⚠️  Failed to install icons:', error.message));
    return false;
  }
}
...
`;
    ...
    // 10. Install IKEA SVG Icons to public folder
    await installIkeaIcons(appPath);
    ...
    console.log(chalk.gray('🎨 Icons: 600+ SVG icons available in public/Icons/'));
