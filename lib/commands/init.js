...
async function initCommand(options) {
  ...
  const isNonInteractive = options.nonInteractive || process.env.UX_INGKA_POSTINSTALL === 'true';
    ...
    if (!isNonInteractive) {
      const { repoAction } = await inquirer.prompt([{
        ...
        choices: [
          ...
          { name: '⏭️  Continue without GitHub features (local only)', value: 'skip' },
          { name: '🔐 Login to GitHub first (gh auth login)', value: 'login' },
          { name: '❌ Exit and configure manually', value: 'exit' }
        ]
      }]);
      ...
      if (repoAction === 'login') {
        ...
        try {
          execSync('gh auth login', { stdio: 'inherit' });
          // Retry getting repo info after login
          try {
            const repoUrl = execSync('gh repo view --json nameWithOwner -q .nameWithOwner', {
              encoding: 'utf8'
            }).trim();
            ...
            console.log(chalk.yellow('\n⚠️  Still not in a GitHub repository. Continuing without GitHub features.\n'));
            skipGitHubFeatures = true;
          }
        } catch (e) {
          console.log(chalk.yellow('\n⚠️  Login cancelled. Continuing without GitHub features.\n'));
          skipGitHubFeatures = true;
        }
      }

      if (repoAction === 'create') {
        const { repoName, repoVisibility, repoDescription } = await inquirer.prompt([
          {
            type: 'input',
            name: 'repoName',
          ...
          },
          {
            ...
            default: 'Created with UX Ingka Kit'
          }
        ]);
        ...
        try {
          ...
          const createCmd = `gh repo create ${repoName} --${repoVisibility} --description "${repoDescription}" --source=. --remote=origin`;
          ...
          isNewRepo = true;
          console.log(chalk.green(`\n✅ Repository created: ${repoInfo}\n`));
        } catch (e) {
          console.log(chalk.red('\n❌ Failed to create repository:', e.message));
          console.log(chalk.yellow('Continuing without GitHub features.\n'));
          skipGitHubFeatures = true;
        }
      }

      if (repoAction === 'skip') {
        console.log(chalk.yellow('\n⏭️  Skipping GitHub features. You can set up a repository later.\n'));
        skipGitHubFeatures = true;
      }
    } else {
      // Non-interactive mode: skip GitHub features
      console.log(chalk.yellow('Non-interactive mode: Skipping GitHub features.\n'));
      skipGitHubFeatures = true;
    }
  }
  ...
  } else if (skipGitHubFeatures) {
    console.log(chalk.gray('Running in local-only mode (no GitHub repository)\n'));
  }
  ...
  if (isNonInteractive) {
    ...
    console.log(chalk.gray(`  • Repository: ${repoInfo || 'None (local only)'}`));
    console.log(chalk.gray('  • Project: Skip (can be configured later)'));
    console.log(chalk.gray(`  • Labels: ${skipGitHubFeatures ? 'Skip (no repo)' : 'Install standard labels'}`));
    console.log(chalk.gray('  • VS Code: Configure with Copilot instructions\n'));

    // Set defaults for non-interactive mode
    config.skipProject = true;
    config.skipLabels = skipGitHubFeatures; // Skip labels if no GitHub
    config.skipVscode = false;
    if (isOrgRepo && repoInfo) {
      config.org = repoInfo.split('/')[0];
    }
  } else if (!skipGitHubFeatures) {
    // Only prompt for GitHub features if we have a repository
    ...
    // Skip GitHub-specific prompts when no repo
    config.skipProject = true;
    config.skipLabels = true;
    console.log(chalk.gray('GitHub features skipped (no repository)\n'));
  }

  // Handle project setup based on choice (only if GitHub features enabled)
  if (!skipGitHubFeatures && config.projectChoice === 'existing') {
  ...
  } else if (!skipGitHubFeatures && config.projectChoice === 'new') {
    const repoName = repoInfo ? repoInfo.split('/')[1] : 'My Project';
    const { projectName, projectDescription } = await inquirer.prompt([
      {
        type: 'input',
        name: 'projectName',
        message: 'Enter new project name:',
        default: `${repoName} - UX Ingka Workflow`,
        validate: (input) => {
      ...
      },
      {
        ...
        default: 'Spec-driven development workflow with UX Ingka Kit'
      }
    ]);
...
`;
  ...
  // Step 3: Set up labels (if not skipped and GitHub features available)
  if (!config.skipLabels && !skipGitHubFeatures) {
  ...
  } else if (skipGitHubFeatures && !config.skipLabels) {
    console.log(chalk.gray('⏭️  Skipping labels (no GitHub repository)'));
  }
  ...
  if (!isNonInteractive) {
    ...
    console.log(chalk.gray('UX Ingka Kit supports multiple AI coding assistants. Select the ones you use:\n'));
  ...
  if (selectedAIs.length > 0) {
    ...
    try {
      ...
      // Save design system configuration to .ux-ingkarc.json (NEW - v6.3.0)
      ...
      if (enableIKEA || enableCWDS) {
        ...
        console.log(chalk.green('\n✅ Design system configuration saved to .ux-ingkarc.json\n'));
      }
      ...
      // Generate modular instruction files (lean orchestrator + agent files)
      // This creates:
      // - .github/copilot-instructions.md (lean orchestrator ~400 lines)
      // - .github/instructions/*.instructions.md (agent files)
      const modularResult = await builder.generateAndWriteModular(selectedAIs, projectType, generationConfig);

      const successCount = modularResult.success.length;
      const failedCount = modularResult.failed.length;
      const agentFilesCount = modularResult.agentFiles.length;

      if (successCount > 0) {
        aiSpinner.succeed(`Generated modular AI instruction files`);

        // Show what was created
        console.log(chalk.gray(`  Main file: .github/copilot-instructions.md (lean orchestrator)`));
        if (agentFilesCount > 0) {
          console.log(chalk.gray(`  Agent files: ${agentFilesCount} files in .github/instructions/`));
        }
        ...
        // Install CWDS components if enabled (NEW - v6.1.0, Updated v6.4.0, v7.0.2)
        if (installCWDSComponents) {
          ...
          try {
            ...
            console.log(chalk.green('\n  ✅ Components ready! When you install the Icon component,'));
            console.log(chalk.green('     600+ IKEA SVG icons will be copied to public/Icons/.'));
          } catch (error) {
  ...
  if (isNewRepo && config.newProjectName) {
    ...
    try {
      ...
      // Create new project with UX Ingka Workflow template
      ...
      if (projectUrlMatch) {
        ...
        // Set up project with UX Ingka Workflow columns
        const setupColumns = [
          '📋 Backlog',
          '🎯 Ready',
          '🚀 In Progress',
  ...
  console.log(chalk.cyan('🎯 The UX Ingka Workflow:\n'));
    ...
    const { setupGitHub } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'setupGitHub',
        message: 'Configure GitHub repository with UX Ingka recommended settings?',
        default: false
      }
    ]);
